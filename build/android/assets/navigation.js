function Navigation(_args) {
    ...
    this.openMenuLink = function(_currentScreen, _nextScreen, _firstPage, _controllerArguments) {
        var _controller;
        "prescriptions" == _currentScreen ? that.prescriptionControllers = that.controllers : "careteam" == _currentScreen ? that.doctorControllers = that.controllers : "stores" == _currentScreen ? that.storeControllers = that.controllers : "account" == _currentScreen && (that.accountControllers = that.controllers);
        that.controllers = [];
        "prescriptions" == _nextScreen ? that.controllers = that.prescriptionControllers : "careteam" == _nextScreen ? that.controllers = that.doctorControllers : "stores" == _nextScreen ? that.controllers = that.storeControllers : "account" == _nextScreen && (that.controllers = that.accountControllers);
    ...
    this.close = function(_callback) {
        ...
        incomingController = 1 == that.controllers.length ? Alloy.createController("landingPage") : that.controllers[that.controllers.length - 2];
